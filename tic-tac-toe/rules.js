...
function begin_play(){
	...
	// get the inputs for each player
	var p1 = document.getElementById("player1_id");
	var p2 = document.getElementById("player2_id");

	// determine if player names are empty
	if (p1.value == "" || p2.value == "") {
		alert("Please enter names for both players");
		return; // if names not entered, do not start the game
	}

	// disable name fields and update with player moves
	p1.disabled = true;
	p2.disabled = true;
	p1.value += " (X)";
	p2.value += " (O)";
...
*/
function reset_play(){
	// get the elements needed multiple times
	var p1 = document.getElementById("player1_id");
	var p2 = document.getElementById("player2_id");
...
function winner(player) {
	var announcement = "The winner is " + player + "!";
	alert(announcement);

	reset_play();
}

function sameString(s1, s2) { // returns 1 if same string, 0 is not
	return (s1<s2?0:(s1>s2?0:1))
}

function threeInARow(c1Id, c2Id, c3Id) {
	var c1 = document.getElementById(c1Id).innerHTML;
	var c2 = document.getElementById(c2Id).innerHTML;
	var c3 = document.getElementById(c3Id).innerHTML;

	if (sameString(c1, c2) && sameString(c2, c3)) { // all of the same element, there is a winner
		if (sameString(c1, "X")) setTimeout(winner, 100, "X"); // p1 is the winner
		else setTimeout(winner, 100, "O"); // p2 is the winner
	}
}

function play() {
	// ensure game is being played first 
	if (!game_started()) {
		alert("The game has not started.");
		return;
	}

	// get the value inputted by the player
	var move = document.getElementById("move_text_id").value;

	// determine if the move is valid
	var validMove = false;
	for (id of table_ids) {
		var dest = document.getElementById(id).innerHTML;
		if (dest == move) validMove = true;
	}
	if (!validMove) {
		alert("Invalid move, please enter the value of an unused cell.");
		return;
	}

	// update the cell with the player's move and change turns
	if (whose_move() == 1) { // p1 turn
	...
	threeInARow("A1", "A2", "A3");
	threeInARow("B1", "B2", "B3");
	threeInARow("C1", "C2", "C3");
	threeInARow("A1", "B1", "C1");
	threeInARow("A2", "B2", "C2");
	threeInARow("A3", "B3", "C3");
	threeInARow("A1", "B2", "C3");
	threeInARow("A3", "B2", "C1");
}
